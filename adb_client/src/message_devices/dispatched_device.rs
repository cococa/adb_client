...
use rand::RngExt;
use std::{collections::HashMap, io::{Read, Write}, collections::VecDeque, net::{TcpListener, TcpStream}, sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}}, sync::mpsc};
...
impl DispatchedADBMessageDevice {
    ...
    fn open_destination(&self, destination: &str) -> Result<DispatchedService> {
        ...
        payload.push(0);
        session.send(ADBTransportMessage::try_new(MessageCommand::Open, local_id, 0, &payload)?)?;
        let response = session.receive()?;
        if response.header().command() != MessageCommand::Okay || response.header().arg1() != local_id {
            return Err(RustADBError::ADBRequestFailed("forward destination rejected".into()));
        }
        Ok(DispatchedService { session, remote_id: response.header().arg0() })
    }
    ...
    pub(crate) fn forward_routes(&self) -> Vec<(String, String)> {
        self.forwards.lock().unwrap().iter().map(|(local, (remote, _))| (local.clone(), remote.clone())).collect()
    }

    pub(crate) fn remove_forward(&self, local: &str) -> bool {
        self.forwards.lock().unwrap().remove(local)
            .map(|(_, stop)| { stop.store(true, Ordering::Release); true }).unwrap_or(false)
    }
    ...
    fn forward_connection(&self, mut socket: TcpStream, remote: &str) -> Result<()> {
        ...
        loop {
            while let Ok(event) = rx.try_recv() {
                match event {
                    None => { let _ = service.close_ack(); return Ok(()); }
                    Some(payload) if !pending => {
                        service.session.send(ADBTransportMessage::try_new(MessageCommand::Write, service.session.local_id(), service.remote_id, &payload)?)?;
                        pending = true;
                    }
                    Some(_) => {}
                }
            }
            match service.session.receive_timeout(std::time::Duration::from_millis(10))? {
                Some(packet) => match packet.header().command() {
                    MessageCommand::Write => {
                        socket.write_all(packet.payload())?;
                        service.acknowledge()?;
                    }
                    MessageCommand::Okay => { pending = false; }
                    MessageCommand::Clse => { let _ = service.close_ack(); return Ok(()); }
                    _ => return Err(RustADBError::ADBRequestFailed("unexpected forward packet".into())),
                },
                None => {},
            }
        }
    ...
    pub(crate) fn shell_command(
        ...
        let service = self.open_session(&ADBLocalCommand::ShellCommand(
            command.to_owned(), vec!["v2".to_owned()],
        ))?;
        ...
        loop {
            ...
            if length > 1024 * 1024 { return Err(RustADBError::ADBRequestFailed("oversized shell packet".into())); }
            let data = service.read_bytes(&mut buffered, length)?;
            match header[0] {
                1 => if let Some(output) = stdout.as_deref_mut() { output.write_all(&data)?; },
                2 => {
                    // Bound diagnostics without blocking or mixing stderr into binary data.
                    if errors.len() < 64 * 1024 { errors.extend_from_slice(&data); }
                },
                3 => {
                    service.close_ack()?;
                    return match data.first() {
                        Some(0) => Ok(()),
                        code => Err(RustADBError::ADBRequestFailed(format!("remote command exited {code:?}: {}", String::from_utf8_lossy(&errors)))),
                    };
                }
                _ => return Err(RustADBError::ADBRequestFailed("invalid shell packet".into())),
            }
        }
    }
    ...
    pub(crate) fn pull(&self, path: &str, output: &mut dyn Write) -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            loop {
                ...
                match &header[..4] {
                    b"DONE" => return Ok(()),
                    b"DATA" if length <= 64 * 1024 => output.write_all(&service.read_bytes(&mut buffered, length)?)?,
                    b"FAIL" if length <= 64 * 1024 => return Err(RustADBError::ADBRequestFailed(String::from_utf8_lossy(&service.read_bytes(&mut buffered, length)?).into_owned())),
                    _ => return Err(RustADBError::ADBRequestFailed("invalid sync download packet".into())),
                }
            }
        })();
    ...
    pub(crate) fn exec_out(&self, command: &str, output: &mut dyn Write) -> Result<()> {
        self.open_session(&ADBLocalCommand::Exec(command.to_owned()))?.drain_to(&mut Some(output))
    }
    ...
    pub(crate) fn reverse_forward(&self, remote: String, local: String) -> Result<()> {
        ...
        if result.is_err() { let _ = self.dispatcher.set_route(remote, None); }
        result
    }
    ...
    pub(crate) fn push<R: Read>(&self, mut input: R, path: &str) -> Result<()> {
        ...
        if !result.payload().starts_with(b"OKAY") {
            return Err(RustADBError::ADBRequestFailed(format!("sync push failed: {}", String::from_utf8_lossy(result.payload()))));
        }
        Ok(())
    }
    ...
    fn reverse_command(&self, command: &ADBLocalCommand) -> Result<()> {
        ...
        self.open_session(command)?.drain_to(&mut Some(&mut response))?;
        if !response.starts_with(b"OKAY") {
            return Err(RustADBError::ADBRequestFailed(format!("reverse failed: {}", String::from_utf8_lossy(&response))));
        }
        Ok(())
    }

    pub(crate) fn is_alive(&self) -> bool { self.dispatcher.is_alive() }
    ...
    pub(crate) fn root(&self) -> Result<()> {
        self.open_session(&ADBLocalCommand::Root)?.drain_to(&mut None)
    }

    pub(crate) fn remount(&self) -> Result<()> {
        self.open_session(&ADBLocalCommand::Remount)?.drain_to(&mut None)
    }
}
        ...
        while buffered.len() < length {
            ...
            match packet.header().command() {
                ...
                MessageCommand::Clse => {
                    ...
                    return Err(RustADBError::ADBRequestFailed("device closed service before completion".into()));
                }
                _ => return Err(RustADBError::ADBRequestFailed("unexpected service packet".into())),
            }
        }
